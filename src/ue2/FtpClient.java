...
//		1)If the source file parent is not null
		if(sinkDirectory != null){
//			2) issue a CWD message to the FTP server using sendRequest(), 
//			setting it's current working directory to the source file parent.
			FtpResponse response = sendRequest("CWD" + sinkDirectory.toString());
			if(response.getCode()!=250){
				throw new NotDirectoryException(response.toString());
			}
//			3) Send a PASV message to query the socket-address to be used for the data transfer
			response = sendRequest("PASV");
			
//			4) ask the response for the socket address returned.
			InetSocketAddress isa = response.getSocketAddress();
			int port = isa.getPort();
			String host = isa.getHostName();
			
//			5) Open a data connection to the socket-address using "new Socket(host, port)".
	...
	public synchronized void sendFile (final Path sourceFile, final Path sinkDirectory) throws IOException {
		...
		if(sinkDirectory!= null){
			FtpResponse response = this.sendRequest("CWD " + sinkDirectory.toString());
			if(response.getCode() == 250){
				response = this.sendRequest("PASV"); //Fehlt evtl. noch was !!!!
				if(response.getCode() == 227){
					InetSocketAddress isa = response.getSocketAddress();
					int port = isa.getPort();
					String host = isa.getHostName();
					try(Socket dataConnection = new Socket(host, port)){
						response = sendRequest("STOR");
						if(response.getCode() == 150){
							try(InputStream inputS = Files.newInputStream(sourceFile)){
								OutputStream outputS = dataConnection.getOutputStream();
								Streams.copy(inputS, outputS, 111111);
							}
						}else{
			...
			//Wenn nicht der Code dann FEHLER
